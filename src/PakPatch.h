    ...
    if (buffer) {
      ...
      TraversalGZIPFile((BYTE*)buffer, [=](uint8_t* begin, uint32_t size,
        ...
        if (pos) {
          ...
          const char prouct_title[] = u8R"({aboutBrowserVersion}</div><div class="secondary"><a target="_blank" href="https://github.com/mitis1233/chrome_plus">Chrome++</a> )" RELEASE_VER_STR u8R"( modified version</div>)";
